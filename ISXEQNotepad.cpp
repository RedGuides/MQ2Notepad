...
#include "ISXEQNotepad.h"

// The mandatory pre-setup function.  Our name is "ISXEQNotepad", and the class is ISXEQNotepad.
...
// our instanced class.
ISXPreSetup("ISXEQNotepad",ISXEQNotepad);
...
unsigned int ISXEQNotepadXML=0;
...
bool ISXEQNotepad::Initialize(ISInterface *p_ISInterface)
...
WriteChatf("ISXEQNotepad Loaded");
return true;
}

// shutdown sequence
void ISXEQNotepad::Shutdown()
{
    // save settings, if you changed them and want to save them now.  You should normally save
    // changes immediately.
    //pISInterface->ExportSet(ISXEQNotepadXML,XMLFileName);

    pISInterface->UnloadSet(ISXEQNotepadXML);
...
void ISXEQNotepad::ConnectServices()
{
...
void ISXEQNotepad::RegisterCommands()
{
    // add any commands
...
void ISXEQNotepad::RegisterAliases()
{
    // add any aliases
}

void ISXEQNotepad::RegisterDataTypes()
{
    // add any datatypes
    // pMyType = new MyType;
...
void ISXEQNotepad::UnRegisterCommands()
{
    // remove commands
    //    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQNotepad::UnRegisterAliases()
{
    // remove aliases
}
void ISXEQNotepad::UnRegisterDataTypes()
{
    // remove data types
    //if (pMyType)
    //{
...
void ISXEQNotepad::UnRegisterTopLevelObjects()
{
    // remove Top-Level Objects
    //    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQNotepad::UnRegisterServices()
{
    // shutdown our own services
    //    if (hISXEQNotepadService)
    //        pISInterface->ShutdownService(this,hISXEQNotepadService);
}

void ISXEQNotepad::LoadSettings()
{
    bool bChanged=false;

    // open the settings file, and store the ID number in ISXEQNotepadXML
    ISXEQNotepadXML=pISInterface->OpenSettings(XMLFileName);
    ...
    Any set can be referred to by its ID.  The base set is stored in ISXEQNotepadXML.  Note that the settings
    ...
    */
    ...
    if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQNotepadXML,"General"))
    {
